 ...
 * Some of these methods (FAST, AGAST, GFTT and MSER) are not available
 ...
 */
async function getImagesMatches (imgData1, imgData2, options = {}) {
  initOpenCV();

  const {detectorName='ORB', visualize=false} = options;
  ...
  const img1 = await cv.imdecodeAsync(imgData1);
  const img2 = await cv.imdecodeAsync(imgData2);
  const [result1, result2] = await B.all([
    detectAndCompute(img1, detector),
    detectAndCompute(img2, detector)
  ]);
  const matches = await cv.matchBruteForceAsync(result1.descriptor, result2.descriptor);
  const result = {
...
async function getImagesSimilarity (img1Data, img2Data, options = {}) {
  ...
  let template = await cv.imdecodeAsync(img1Data);
  let reference = await cv.imdecodeAsync(img2Data);
