 ...
 * @property {Array<OccurrenceResult>} results The array of matching OccurenceResults
...
async function getImageOccurrences (fullImgData, partialImgData, options = {}) {
  ...
  try {
    ...
    for (let element of nonZeroMatchResults) {
      let hasNoNeighbours = true;

      matches.some((match) => {
        const distanceBetweenPoints = distance(match, element);
        if (distanceBetweenPoints <= MATCH_NEIGHBOUR_THRESHOLD) {
