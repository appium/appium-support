 ...
 * Some of these methods (FAST, AGAST, GFTT, SIFT and MSER) are not available
 * in the default OpenCV installation and have to be enabled manually before
 * compilation.
 * @property {?string} matchFunc ['matchBruteForce'] The name of the matching function.
 * Available names are: matchFlannBased, matchBruteForce, matchBruteForceL1,
 * matchBruteForceHamming, matchBruteForceHammingLut, matchBruteForceSL2
 ...
 * minimal distance, maximum distance and returns true or false.
 ...
 * the more mctahing edges there are no both images the more similar they are.
...
async function getImagesMatches (img1Data, img2Data, options = {}) {
  ...
  const {detectorName = 'ORB', visualize = false,
         goodMatchesFactor, matchFunc = 'matchBruteForce'} = options;
  ...
  let matches = [];
  try {
    matches = await cv[`${matchFunc}Async`](result1.descriptor, result2.descriptor);
  } catch (e) {
  ...
  if (visualize) {
    ...
    const drawMatchedRect = (mat, rect) => {
      ...
      const thickness = 2;
      mat.drawRectangle(new cv.Rect(rect.x, rect.y, rect.width, rect.height), color, thickness, cv.LINE_8);
    };
    drawMatchedRect(visualization, rect1);
    drawMatchedRect(visualization, {
      x: img1.cols + rect2.x,
      y: rect2.y,
      width: rect2.width,
      height: rect2.height
...
async function getImagesSimilarity (img1Data, img2Data, options = {}) {
  ...
  if (visualize) {
    ...
    for (const contour of contours) {
      ...
      const color = new cv.Vec(0, 0, 255);
      const thickness = 2;
      resultMat.drawRectangle(boundingRect, color, thickness, cv.LINE_8);
      resultMat.drawRectangle(
        new cv.Rect(reference.cols + boundingRect.x, boundingRect.y, boundingRect.width, boundingRect.height),
 ...
 * @property {Point} location The absolute coordinates of the partial image occurence
 * @property {?Buffer} visualization The visualization of the matching result
...
async function getImageOccurrence (fullImgData, partialImgData, options = {}) {
  ...
  if (visualize) {
    ...
    const color = new cv.Vec(0, 0, 255);
    const thickness = 2;
    fullImg.drawRectangle(
      new cv.Rect(result.location.x, result.location.y, partialImg.cols, partialImg.rows),
      color, thickness, cv.LINE_8);
