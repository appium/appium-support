 ...
 * @property {?int} match_neighbour_threshold [10] The pixel distance between matches we consider
...
async function getImageOccurrence (fullImgData, partialImgData, options = {}) {
  ...
  const { visualize = false,
          ...
          multiple = false,
          match_neighbour_threshold = MATCH_NEIGHBOUR_THRESHOLD } = options;
  ...
  try {
    ...
    if (multiple) {
      ...
      const matches = filterNearMatches(nonZeroMatchResults, match_neighbour_threshold);
    ...
    if (minMax.maxVal < threshold || results.length === 0) {
 ...
 *
 * @param {Mat} nonZeroMatchResults matrix of image match results
 * @param {int} match_neighbour_threshold The pixel distance within which we
 * consider an element being a neighbour of an existing match
 * @return {Array<Point>} the filtered array of matched points
 */
function filterNearMatches (nonZeroMatchResults, match_neighbour_threshold) {
  const matches = [];

  for (const element of nonZeroMatchResults) {
    if (!matches.some((match) => distance(match, element) <= match_neighbour_threshold)) {
