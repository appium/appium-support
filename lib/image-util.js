...
function cropImage (image, rect) {
  ...
  if (isEmptyRect(interRect)) {
    throw new Error(`Cannot crop ${JSON.stringify(rect)} from ${JSON.stringify(imageRect)} because the intersection between them is an empty rectangle`);
  }
...
function getRectIntersection (rect, imageSize) {
  ...
  // if we can't find a total intersection of rect with imageSize, then say
  // that width and/or height are 0, to indicate we didn't get a valid
  // intersection
