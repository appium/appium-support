...
/**
 * @typedef {Object} OccurrenceResults
 * @property {OccurrenceResult[]} results The array of matching OccurenceResults
 * @property {?Buffer} visualization The visualization of the matching results
 * represented as PNG image buffer. On this image the matching
 * regions are highlighted with a rectangle.
 */
 ...
 * @param {?OccurrenceOptions} options [{}] Set of occurrence calculation options
 *
 * @returns {OccurrenceResults}
 * @throws {Error} If no occurences of the partial image can be found in the full image
 */
async function getImageOccurrences (fullImgData, partialImgData, options = {}) {
  await initOpenCV();
  ...
  try {
    ...
    let nonZero = matched.threshold(threshold, 1, cv.THRESH_BINARY).convertTo(cv.CV_8U).findNonZero();
    let arr = matched.getDataAsArray();

    let matches = [];

    nonZero.forEach((element) => {
      let hasNoNeighbours = true;

      matches.some((match) => {
        let distanceBetweenPoints = distance(match, element);
    ...
    matches.forEach((element) => {
      let score = arr[element.y][element.x];
      let result = {};
  ...
  if (visualize) {
    ...
    visualization = await cv.imencodeAsync('.png', fullImg);
  }
...
function distance (point1, point2) {
  let a2 = Math.pow((point1.x - point2.x), 2);
  let b2 = Math.pow((point1.y - point2.y), 2);
...
export {
  ...
  getImagesSimilarity, getImageOccurrence, getImageOccurrences, getJimpImage, MIME_JPEG, MIME_PNG,
  MIME_BMP,
};
