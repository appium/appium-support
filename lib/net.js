...
async function uploadFileToHttp (localFileStream, remoteUrl, uploadOptions = {}) {
  ...
  log.debug(`${remoteUrl.protocol} upload options: ${JSON.stringify(requestOpts)}`);
...
async function uploadFileToFtp (localFileStream, remoteUrl, uploadOptions = {}) {
  ...
  log.debug(`${remoteUrl.protocol} upload options: ${JSON.stringify(ftpOpts)}`);
  return await new B((resolve, reject) => {
    new Ftp(ftpOpts).put(localFileStream, remoteUrl.pathname, (err) => {
      if (err) {
        reject(err);
      } else {
        resolve();
 ...
 * @property {Array[Array<Object>]|Object} formFields - The additional form fields
