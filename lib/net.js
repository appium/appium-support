...
async function uploadFileToHttp (localFileStream, parsedUri, uploadOptions = {}) {
  ...
  if (fileFieldName) {
    ...
    log.debug(`${protocol} upload options (excluding form data): ${JSON.stringify(requestOpts)}`);
    ...
    log.debug(`${protocol} upload options (excluding data): ${JSON.stringify(requestOpts)}`);
 ...
 * content to be uploaded. `null` value makes the request to use non-multipart upload
