...
class SecureValuePreprocessingRule {
  constructor (pattern, replacer = DEFAULT_SECURE_REPLACER) {
...
class SecureValuesPreprocessor {
  ...
  parseRule (rule) {
    if (_.isString(rule)) {
      return new SecureValuePreprocessingRule(new RegExp(`\\b${_.escapeRegExp(rule)}\\b`, 'g'));
    }
    ...
    if (_.isPlainObject(rule)) {
      ...
      if (!pattern) {
        throw new Error(`The '${JSON.stringify(rule)}' rule must either have a field named 'pattern' or 'text'`);
      }
      ...
      try {
        return new SecureValuePreprocessingRule(new RegExp(pattern, flags.join('')), replacer);
      } catch (e) {
        throw new Error(`'${rule.pattern}' must be a valid RegExp pattern: ${e.message}`);
      }
    }
   ...
   *
   * @param {string} srcPath The full path to the JSON file containing secure
   * values replacement rules
   ...
   */
  async loadRules (srcPath) {
    if (!await fs.exists(srcPath)) {
      throw new Error(`'${srcPath}' does not exist or is not accessible`);
    }
    let rules;
    try {
      rules = JSON.parse(await fs.readFile(srcPath, 'utf8'));
    } catch (e) {
      throw new Error(`'${srcPath}' must be a valid JSON file. Original error: ${e.message}`);
    }
    if (!_.isArray(rules)) {
      throw new Error(`'${srcPath}' must contain a valid JSON array`);
    }
